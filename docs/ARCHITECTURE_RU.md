...
| Субъект 42 | Один хост; без reverse DNS в ответах; вывод как у inetutils |
...
`timeout_reached` сравнивает **только секунды** (`tv_sec`), без учёта микросекунд — достаточно для субъекта.
...
|------|---------------|------------|
| `-v` | `options \|= OPT_VERBOSE` | id в заголовке, дампы ICMP-ошибок |
...
| `-n` | (пока без эффекта) | зарезервировано под numeric-only |
