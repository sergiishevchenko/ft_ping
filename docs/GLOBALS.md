...
Related: [ARCHITECTURE.md](ARCHITECTURE.md), [concepts/SIGNALS.md](concepts/SIGNALS.md), [GETOPT-LONG.md](GETOPT-LONG.md), [FLAGS.md](FLAGS.md).
...
**Count:** 1 external global, 1 `static` file-scope table. All other session state (including `-r` via `ping->dontroute`) lives in `t_ping ping` on the stack in `main()`.
...
2. **State model** — almost everything is in `t_ping` (including `-r` → `ping->dontroute`); globals are exceptions, not the default.
3. **`g_stop`** — unavoidable without a more complex signal setup; handler stays minimal and async-signal-safe.
4. **`g_long_opts`** — static constant table for `getopt_long`; not shared mutable state.
